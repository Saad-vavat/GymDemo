...
# def view_member(request):
    ...
    m_paginator = Paginator(morning,10)
    try:
        page_morning = request.GET.get('page_morning', 1)
        morning = m_paginator.page(page_morning)
    except PageNotAnInteger:
        morning = m_paginator.page(1)
    except EmptyPage:
        morning = m_paginator.page(m_paginator.num_pages)    
    
    evening = Member.objects.filter(batch='evening', stop=0).order_by('first_name')
    e_paginator = Paginator(evening,10)
    try:
        page_evening = request.GET.get('page_evening', 1)
        evening = e_paginator.page(page_evening)
    except PageNotAnInteger:
        evening = e_paginator.page(1)
    except EmptyPage:
        evening = e_paginator.page(e_paginator.num_pages)
     
    stopped = Member.objects.filter(stop=1).order_by('first_name')
    s_paginator = Paginator(stopped,10)
    try:
        page_stopped = request.GET.get('page_stopped', 1)
        stopped = s_paginator.page(page_stopped)
    except PageNotAnInteger:
        stopped = s_paginator.page(1)
    except EmptyPage:
        stopped = s_paginator.page(s_paginator.num_pages)
...
# def view_member(request):
    # Get all active members and order by first name
    view_all = Member.objects.filter(stop=0).order_by('first_name')
    paginator_all = Paginator(view_all, 1)

    # Get morning members and order by first name
    morning = Member.objects.filter(batch='morning', stop=0).order_by('first_name')
    paginator_morning = Paginator(morning, 1)

    # Get evening members and order by first name
    evening = Member.objects.filter(batch='evening', stop=0).order_by('first_name')
    paginator_evening = Paginator(evening, 1)

    # Get stopped members and order by first name
    stopped = Member.objects.filter(stop=1).order_by('first_name')
    paginator_stopped = Paginator(stopped, 1)

    # Get the current page number for all active members
    try:
        page_all = request.GET.get('page_all', 1)
        view_all = paginator_all.page(page_all)
    except PageNotAnInteger:
        view_all = paginator_all.page(1)
    except EmptyPage:
        view_all = paginator_all.page(paginator_all.num_pages)

    # Get the current page number for morning members
    try:
        page_morning = request.GET.get('page_morning', 1)
        morning = paginator_morning.page(page_morning)
    except PageNotAnInteger:
        morning = paginator_morning.page(1)
    except EmptyPage:
        morning = paginator_morning.page(paginator_morning.num_pages)

    # Get the current page number for evening members
    try:
        page_evening = request.GET.get('page_evening', 1)
        evening = paginator_evening.page(page_evening)
    except PageNotAnInteger:
        evening = paginator_evening.page(1)
    except EmptyPage:
        evening = paginator_evening.page(paginator_evening.num_pages)

    # Get the current page number for stopped members
    try:
        page_stopped = request.GET.get('page_stopped', 1)
        stopped = paginator_stopped.page(page_stopped)
    except PageNotAnInteger:
        stopped = paginator_stopped.page(1)
    except EmptyPage:
        stopped = paginator_stopped.page(paginator_stopped.num_pages)

    # Create a search form
    search_form = SearchForm()

    # Create the context dictionary with the paginated data and the search form
    context = {
        'all': view_all,
        'morning': morning,
        'evening': evening,
...
def view_member(request):
    ...
    if query:
        view_all = view_all.filter(Q(first_name__icontains=query) | Q(last_name__icontains=query) | Q(email__icontains=query))
        morning = morning.filter(Q(first_name__icontains=query) | Q(last_name__icontains=query) | Q(email__icontains=query))
        evening = evening.filter(Q(first_name__icontains=query) | Q(last_name__icontains=query) | Q(email__icontains=query))
        stopped = stopped.filter(Q(first_name__icontains=query) | Q(last_name__icontains=query) | Q(email__icontains=query))
    
    paginator = Paginator(view_all, 10)
    page_number = request.GET.get('page', 1)
    view_all = paginator.get_page(page_number)

    m_paginator = Paginator(morning,10)
    page_number_morning = request.GET.get('page_morning', 1)
    morning = m_paginator.get_page(page_number_morning)

    e_paginator = Paginator(evening,10)
    page_number_evening = request.GET.get('page_evening', 1)
    evening = e_paginator.get_page(page_number_evening)

    s_paginator = Paginator(stopped,10)
    page_number_stopped = request.GET.get('page_stopped', 1)
    stopped = s_paginator.get_page(page_number_stopped)
